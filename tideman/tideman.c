...
// a get_string. If it finds a match it adds that candidate's array position (which will work as ID)
...
// candidate pairs (in prefrences[i][j]) where i is the winner and j the loser candidate. In this
// function there's no need to call again the candidates positions as the array ranks already
// registered. For example, ranks[0] will have registered some candidade (i), and since rank[0]
...
// stength pair[0]. which translated is: is preferences[pairs[1].winner][pairs[1].loser] >
...
// point and an target point. by using the created pairs (now in order of strenght) we start from 0
...
// -1 The logic behind this is the follwing: we check if have previously locked a pair where the
// loser have won. If has won, we then check if the loser from that is the same that is same winner
...
// locked_pairs (true and falses) in a matrix, we would notice the the source of the graph would be
// the one candidate that was never in a lose positions, in other words, we check every lose for
